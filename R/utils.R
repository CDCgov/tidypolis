...
#' @keywords internal
get_env_site_data <- function(file_loc = file.path(
                                Sys.getenv("POLIS_DATA_FOLDER"),
                                "misc",
                                "env_sites.rds"
                              )) {
  invisible(capture.output(
    envSiteYearList <- tidypolis_io(
      io = "read",
      file_path = file_loc
    )
  ))
...
f.compare.dataframe.cols <- function(old, new) {
  ...
  if (length(setdiff(colnames(new), colnames(old))) >= 1 | length(setdiff(colnames(old), colnames(new))) >= 1) {
    ...
    update_polis_log(
      .event = paste0("New Var: ", new.var, ", Removed Var: ", lost.var),
      .event_type = "ALERT"
    )
  } else {
    cli::cli_alert_info("No differences found in column names")
  }
}
...
f.download.compare.02 <- function(df.from.f.download.compare.01,
  ...
  for (i in 1:length(x)) {
    var.check <- dplyr::anti_join(
      ...
      (old.download |> dplyr::select(x[i]) |> dplyr::distinct()),
      by = x[i]
    )
...
f.compare.metadata <- function(new_table_metadata, old_table_metadata, table) {
  ...
  if (length(new_vars) != 0) {
    ...
    update_polis_log(
      .event = paste0(table, " - ", "New Var(s): ", paste(new_vars, collapse = ", ")),
      .event_type = "ALERT"
    )
  }
  ...
  if (length(lost_vars) != 0) {
    ...
    update_polis_log(
      .event = paste0(table, " - ", "Lost Var(s): ", paste(lost_vars, collapse = ", ")),
      .event_type = "ALERT"
    )
  }
  ...
  if (length(new_vars) == 0 & length(lost_vars) == 0) {
    update_polis_log(
      .event = paste0(table, " - ", "No new or lost variables"),
      .event_type = "INFO"
    )
  }
  ...
  new_response <- compare_metadata |>
    ...
    dplyr::rowwise() |>
    dplyr::mutate(
      same = toString(intersect(categorical_response_set.x, categorical_response_set.y)),
    ...
    dplyr::rename(
      old_categorical_response_set = categorical_response_set.x,
...
#'
#' @description Read log entries from the latest download and preprocessing run,
...
archive_log <- function(log_file = Sys.getenv("POLIS_LOG_FILE"),
  ...
  if (!flag.log.exists) {
    tidypolis_io(
      io = "write", obj = log.to.arch,
      file_path = file.path(polis_data_folder, "Log_Archive/log_archive.rds")
    )
  } else {
    log_archrive <- tidypolis_io(
      io = "read",
      file_path = file.path(polis_data_folder, "Log_Archive/log_archive.rds")
    )
  ...
  if (type == "ES") {
    ...
    df.01.fixed <- df.01 |>
      dplyr::mutate(dplyr::across(
        dplyr::contains("date") & dplyr::where(is.character),
...
create_response_vars <- function(pos,
  ...
  sia.sub <- sia |>
    dplyr::select(
      sia.code, sia.sub.activity.code, activity.start.date,
      sub.activity.start.date, vaccine.type, adm2guid
    )

  pos.sub <- pos |>
    dplyr::select(
      epid, dateonset, yronset, measurement, ntchanges, emergencegroup,
  ...
  pos.sub.03 <- dplyr::left_join(pos.sub.02, ipv.response, by = c("epid", "dateonset", "ntchanges", "emergencegroup")) |>
    dplyr::mutate(
      finished.responses = ifelse(is.na(finished.responses), 0, finished.responses),
...
cluster_dates <- function(x,
  ...
  if (method == "kmeans") {
    ...
    if (method == "mindate") {
      x <- x |>
        dplyr::mutate(
          cluster = as.numeric(sub.activity.start.date - min(sub.activity.start.date)),
...
check_missingness <- function(data,
  ...
  if (type == "AFP") {
    afp.vars <- c(
      "notification.date", "investigation.date",
...
preprocess_cdc <- function(polis_folder = Sys.getenv("POLIS_DATA_FOLDER"),
  ...
  update_polis_log(
    .event = "Creating ES analytic datasets",
    .event_type = "PROCESS"
  )
...
#' Add GPEI website reported cases (DRAFT)
#'
#'
#' @description
...
#' based on lowest level admin data. This function is currently in draft form and isn't
#' intended to be used at this time.
#'
...
add_gpei_cases <- function(azcontainer = suppressMessages(get_azure_storage_connection()),
  ...
  if (nrow(proxy.data.fill.prov) >= 1) {
    ...
    pt01 <- lapply(1:nrow(proxy.data.fill.prov.02), function(x) {
      tryCatch(
        expr = {
          suppressMessages(sf::st_sample(proxy.data.fill.prov.02[x, ], pull(proxy.data.fill.prov.02[x, ], "nperarm"),
            exact = T
          )) |> st_as_sf()
        },
        error = function(e) {
          guid <- proxy.data.fill.prov.02[x, ]$GUID[1]
          ctry_prov_dist_name <- global.prov |>
            filter(GUID == adm1guid) |>
    ...
    pt01_joined <- dplyr::bind_cols(
      ...
      dplyr::left_join(
        dplyr::as_tibble(proxy.data.fill.prov.02) |>
          dplyr::select(-Shape),
        by = "GUID"
      )
  ...
  if (nrow(proxy.data.fill.ctry) >= 1) {
    ...
    pt01 <- lapply(1:nrow(proxy.data.fill.ctry.02), function(x) {
      tryCatch(
        expr = {
          suppressMessages(sf::st_sample(proxy.data.fill.ctry.02[x, ], pull(proxy.data.fill.ctry.02[x, ], "nperarm"),
            exact = T
          )) |> st_as_sf()
        },
        error = function(e) {
          guid <- proxy.data.fill.ctry.02[x, ]$GUID[1]
          ctry_prov_dist_name <- global.ctry |>
            filter(GUID == adm0guid) |>
            select(ADM0_NAME)
          cli::cli_alert_warning(paste0(
            "Fixing errors for:\n",
            "Country: ", ctry_prov_dist_name$ADM0_NAME, "\n"
    ...
    pt01_joined <- dplyr::bind_cols(
      ...
      dplyr::left_join(
        dplyr::as_tibble(proxy.data.fill.ctry.02) |>
          dplyr::select(-Shape),
        by = "GUID"
      )
    ...
    rm(
      proxy.data.fill.ctry, proxy.data.fill.ctry.01, proxy.data.fill.ctry.02,
...
check_missing_static_files <- function(core_files_folder_path,
  ...
  core_files_to_combine <- tidypolis_io(
    io = "list",
...
s1_prep_polis_tables <- function(polis_folder, polis_data_folder,
  ...
  update_polis_log(
    .event = "Beginning Preprocessing - Creation of CORE Ready Datasets",
    .event_type = "START"
  )

  crosswalk_data <- get_crosswalk_data(file.path(
    polis_folder,
    "misc", "crosswalk.rds"
  ))
  ...
  if (archive) {
    s1_archive_old_files(
      polis_data_folder, timestamp,
  ...
  rm(
    api_case_data, api_es_data, api_virus_data, api_activity_data,
    api_subactivity_data
  )
...
s1_clean_case_table <- function(path, crosswalk,
  ...
  api_case_sub2 <- rename_via_crosswalk(
    api_data = api_case_sub1,
    crosswalk = crosswalk,
    table_name = "Case"
  )
  rm(api_case_sub1)
  ...
  api_case_sub3 <- api_case_sub2 |>
    ...
    dplyr::select(dplyr::any_of(c(
      crosswalk$Web_Name[crosswalk$Table %in% c("Case") &
...
s1_clean_es_table <- function(path, crosswalk,
  ...
  cli::cli_process_start("Crosswalk and rename variables")
  api_es_sub2 <- rename_via_crosswalk(
    api_data = api_es_sub1,
    crosswalk = crosswalk,
  ...
  api_es_sub3 <- api_es_sub2 |>
    dplyr::mutate(
      Site = paste0(`Site Code`, " - ", `Site Name`),
...
s1_clean_virus_table <- function(path, crosswalk,
  ...
  cli::cli_process_start("Crosswalk and rename variables")
  api_virus_sub2 <- rename_via_crosswalk(
    api_data = api_virus_sub1,
    crosswalk = crosswalk,
    table_name = "Virus"
  )
  cli::cli_process_done()
  ...
  api_virus_sub3 <- api_virus_sub2 |>
    ...
    dplyr::select(c(
      crosswalk$Web_Name[crosswalk$Table %in% c("Virus") &
        !is.na(crosswalk$Web_Name)],
...
s1_clean_activity_table <- function(path, subactivity_path, crosswalk,
  ...
  invisible(capture.output(
    sia_subactivity_code <- tidypolis_io(
      io = "read",
  ...
  cli::cli_process_start("Crosswalk and rename variables")
  api_activity_sub2 <-
    rename_via_crosswalk(
      api_data = api_activity_sub1,
      crosswalk = crosswalk,
      table_name = "Activity"
    ) |>
    dplyr::select(
      SIASubActivityCode, WHORegion,
...
s1_clean_subactivity_table <- function(path, activity_table, crosswalk,
  ...
  api_subactivity_sub2 <- api_subactivity_sub1 |>
    ...
    dplyr::mutate(
      Admin2Guid = dplyr::case_when(
        is.na(Admin2Guid) ~ GUID,
        TRUE ~ Admin2Guid
      ),
      Admin1Guid = dplyr::case_when(
        is.na(Admin1Guid) ~ ADM1_GUID,
        TRUE ~ Admin1Guid
      )
    ) |>
  ...
  api_subactivity_sub4 <- api_subactivity_sub3 |>
    ...
    dplyr::mutate_at(
      c("Age Group %", "Area Targeted %", "Country Population %"),
      ~ as.numeric(.) * 100
    ) |>
    ...
    dplyr::select(c(
      crosswalk$Web_Name[crosswalk$Table %in% c("Activity", "SubActivity") &
        !is.na(crosswalk$Web_Name)],
...
#' @keywords internal
#'
s1_archive_old_files <- function(polis_data_folder, timestamp, output_folder_name, output_format) {
...
s2_fully_process_afp_data <- function(polis_data_folder, polis_folder,
  ...
  afp_with_guids <- s2_fix_admin_guids(
    data = afp_validated,
...
s2_process_coordinates <- function(data, polis_data_folder, polis_folder,
  ...
  invisible(capture.output(
    shape_data <- switch(Sys.getenv("POLIS_EDAV_FLAG"),
      "TRUE" = {
        sirfunctions::load_clean_dist_sp(fp = file.path(
          "GID/PEB/SIR",
          polis_folder,
          "misc",
          "global.dist.rds"
  ...
  if (nrow(dup_epid) > 0) {
    ...
    dup_epid_fixed <- dup_epid |>
      ...
      dplyr::mutate(epid_fixed = paste0(
        stringr::str_sub(place.admin.0, 1, 3), "-",
...
s3_fully_process_sia_data <- function(long.global.dist.01, polis_data_folder,
  ...
  # final clean dataset
  sia.clean.01 <- s3_sia_combine_historical_data(
    sia.new = sia.06,
    polis_data_folder
  )
  ...
  s3_sia_evaluate_unmatched_guids(
    sia.05 = sia.05, polis_data_folder,
    output_folder_name = output_folder_name
  )
...
s3_sia_check_guids <- function(sia.02, long.global.dist.01) {
  ...
  tofix <- sia.03 |>
    ...
    dplyr::mutate(
      missing.guid = ifelse(is.na(GUID), 1, 0),
  ...
  sia.04 <- sia.03 |>
    ...
    dplyr::mutate(
      place.admin.1 = ifelse(is.na(place.admin.1), ADM1_NAME, place.admin.1),
...
s3_sia_check_duplicates <- function(sia.05) {
  ...
  sia.06 <- sia.05 |>
    ...
    dplyr::mutate(
      `country.population.%` = as.character(`country.population.%`),
...
s3_sia_check_metadata <- function(sia.06, polis_data_folder, latest_folder_in_archive,
  ...
  if (length(old.file) > 0) {
    ...
    sia_metadata_comparison <- f.compare.metadata(
    ...
    update_polis_log(
      .event = paste0(
        "SIA New Records: ", nrow(in_new_not_old), "; ",
...
s3_sia_cluster_dates_by_vax_type <- function(data,
  ...
  if (cache_exists) {
    ...
    invisible(capture.output(
      tidypolis_io(
        obj = dplyr::bind_rows(in_data, cache),
        io = "write",
        file_path = paste0(cache_folder, "/", type, "_cluster_cache.rds")
      )
    ))
  ...
  if (nrow(in_data) > 0) {
    ...
    # data cache
    data_cache_exists <- tidypolis_io(
      io = "exists.file",
...
s4_es_data_processing <- function(es.01.new,
  ...
  es.02 <- es.01.new |>
    ...
    dplyr::mutate(
      ...
      ev.detect = dplyr::case_when(
        (
          dplyr::if_any(c(
            "vaccine.1", "vaccine.2", "vaccine.3", "nvaccine.2",
  ...
  es.dup.01 <- es.02 |>
    ...
    dplyr::select(
      env.sample.manual.edit.id, env.sample.id, sample.id,
  ...
  if (nrow(es.dup.01) >= 1) {
    ...
    cli::cli_alert_warning(
      paste0(
        "Writing out ES duplicates file: ",
        polis_data_folder,
  ...
  na.es.01 <- es.02 |>
    dplyr::summarise_at(
      dplyr::vars(ctry.guid, prov.guid, dist.guid, lat, lng),
      list(~ sum(is.na(.)))
    )
...
s4_es_create_cdc_vars <- function(es.02, polis_folder, output_folder_name) {
  ...
  es.04 <- dplyr::left_join(
    ...
    dplyr::mutate_at(
      c(
        "admin.2.id", "region.id", "reporting.week",
    ...
    tidypolis_io(
      ...
      file_path = paste(
        ...
        paste("es", min(es.05$collect.date, na.rm = T),
          max(es.05$collect.date, na.rm = T),
          sep = "_"
        ),
        output_format,
        sep = ""
...
s5_fully_process_pos_data <- function(polis_folder,
  ...
  afp.es.virus.02 <- remove_character_dates(
    type = "POS", df = afp.es.virus.01,
    output_folder_name = output_folder_name
  )
  afp.es.virus.03 <- create_response_vars(
    pos = afp.es.virus.02,
    output_folder_name = output_folder_name
  )
...
s5_pos_load_data <- function(polis_data_folder, latest_folder_in_archive,
                             output_folder_name) {
  update_polis_log(
    .event = "Creating Positives analytic datasets",
    .event_type = "PROCESS"
  )
  ...
  if (length(old.file) > 0) {
    ...
    var.names.01 <- var.names |>
      dplyr::filter(variable != "virus.type(s)" & variable != "vdpv.classification(s)" & variable != "nt.changes" & variable != "emergence.group(s)" &
        ...
        !(variable %in% c(
          "exact.longitude", "exact.latitude", "pons.latitude", "pons.longitude", "pons.environment",
          "pons.seq.date", "pons.administration.type", "pons.spec.type", "location", "country.iso2",
          "nt.changes", "location"
        ))) # list of variables we want evaluated in 2nd QC function
...
s5_pos_create_cdc_vars <- function(virus.raw.new, polis_folder, polis_data_folder) {
  ...
  nopv.emrg <- tidypolis_io(
    ...
    dplyr::rename(
      emergencegroup = emergence_group,
  ...
  virus.01 <- virus.raw.new |>
    ...
    dplyr::mutate(
      ...
      vaccine.source = ifelse(virus.type %in% c(
        "VACCINE1", "VACCINE2", "VACCINE3", "VDPV1",
        "VDPV2", "VDPV3"
      ), "Sabin", NA),
  ...
  vaccine.6.plus <- virus.01 |>
    dplyr::filter(virustypename %in% c("VACCINE 1", "VACCINE 2", "VACCINE 3", "VACCINE 2-n") &
      ntchanges >= 6) |>
    dplyr::select(
      epid, surveillance.type, virustype, virustypename,
  ...
  if (nrow(vaccine.6.plus) >= 1) {
    ...
    tidypolis_io(
      obj = vaccine.6.plus, io = "write",
      file_path = paste0(polis_data_folder, "/Core_Ready_Files/virus_large_nt_changes.csv")
    )

    update_polis_log(
      .event = paste0("Vaccine viruses with 6+ NT changes, flag for POLIS"),
      .event_type = "ALERT"
    )
  } else {
...
s5_pos_check_duplicates <- function(virus.01, polis_data_folder, output_folder_name) {
  virus.dup.01 <- virus.01 |>
    dplyr::select(
      epid, epid.in.polis, pons.epid, virus.id, polis.case.id, env.sample.id,
  ...
  if (nrow(virus.dup.01) >= 1) {
    ...
    tidypolis_io(
      obj = virus.dup.01, io = "write",
      file_path = paste0(polis_data_folder, "/", output_folder_name, "/duplicate_viruses_Polis_virusTableData.csv")
    )

    update_polis_log(
      .event = paste0("Duplicate viruses available in duplicate_viruses_Polis_virusTableData.csv"),
      .event_type = "ALERT"
    )
  } else {
...
s5_pos_process_human_virus <- function(virus.01, polis_data_folder, output_folder_name,
  ...
  afp.files.01 <- dplyr::tibble("name" = tidypolis_io(io = "list", file_path = file.path(polis_data_folder, output_folder_name), full_names = T)) |>
    dplyr::mutate(short_name = stringr::str_replace(name, paste0(
      polis_data_folder, "/",
      output_folder_name,
      "/"
    ), "")) |>
  ...
  human.virus.02 <- dplyr::right_join(afp.02, human.virus.01, by = c("epid" = "epid")) |>
    ...
    dplyr::distinct() |>
    dplyr::mutate(
      latitude = ifelse(is.na(lat), y, lat),
  ...
  congo.afp.wild1 <- afp.01 |>
    ...
    dplyr::select(
      epid, dateonset, place.admin.0, place.admin.1, place.admin.2, adm0guid, yronset, adm1guid, adm2guid, cdc.classification.all,
      whoregion, nt.changes, emergence.group, virus.cluster, surveillancetypename, latitude, longitude, datasource, virustypename
    ) |>
    dplyr::rename(
      virustype = cdc.classification.all, surveillance.type = surveillancetypename, viruscluster = virus.cluster,
  ...
  human.virus.05 <- human.virus.04 |>
    dplyr::mutate(
      place.admin.0 = stringr::str_trim(place.admin.0, "left"),
...
s5_pos_process_es_virus <- function(virus.01, polis_data_folder, output_folder_name,
  ...
  env.files.01 <- dplyr::tibble("name" = tidypolis_io(
    io = "list",
  ...
  tryCatch(
    {
      es.01 <- purrr::map_df(env.files.01, \(x) {
        ...
        if ("Admin 0 Id" %in% names(df)) {
          df <- df |>
            dplyr::mutate(
              `Admin 0 Id` = as.character(`Admin 0 Id`),
  ...
  es.02 <- es.01 |>
    ...
    dplyr::mutate(
      ...
      datenotificationtohq = parse_date_time(
        date.notification.to.hq,
        c("%Y-%m-%d", "%d/%m/%Y")
      ),
...
#' @keywords internal
#'
s5_pos_compare_with_archive <- function(afp.es.virus.01, afp.es.virus.03, polis_data_folder, latest_folder_in_archive, output_folder_name, output_format) {
  ...
  if (length(old.file) > 0) {
    ...
    if (nrow(in_new_and_old_but_modified) > 0) {
      ...
      if (nrow(pos_changed_virustype) > 0) {
        update_polis_log(
          .event = paste0("Virus type has changed for ", nrow(pos_changed_virustype), " records, review in Changed_virustype_virusTableData.csv"),
          .event_type = "ALERT"
        )
      }
  ...
  update_polis_log(
    .event = paste0(
      "POS New Records: ", nrow(in_new_not_old), "; ",
...
s5_pos_evaluate_unmatched_guids <- function(afp.es.virus.03, long.global.dist.01, polis_data_folder, output_folder_name) {
  ...
  unmatched.afp.es.viruses.01 <- dplyr::anti_join(afp.es.virus.03,
    long.global.dist.01,
    by = c("admin2guid" = "GUID", "yronset" = "active.year.01")
  )

  # CSV file listing out unmatch virus

  tidypolis_io(
    obj = unmatched.afp.es.viruses.01,
    io = "write",
    file_path = paste(polis_data_folder, "/", output_folder_name, "/",
