...
add_rolling_years <- function(df, start_date, end_date, date_col) {
  ...
  df |>
    dplyr::mutate(
      ...
      year_num = floor(date_interval / lubridate::years(1)),
      year_label = paste0("Year ", year_num + 1),
      analysis_year_start = start_date %m+% lubridate::years(year_num),
      analysis_year_end = analysis_year_start %m+% lubridate::years(1) %m-% lubridate::days(1),
      rolling_period = paste0(
        lubridate::month(analysis_year_start, label = TRUE, abbr = TRUE), " ", lubridate::year(analysis_year_start),
        " - ",
        lubridate::month(analysis_year_end, label = TRUE, abbr = TRUE), " ", lubridate::year(analysis_year_end)
      )
    ) |>
    dplyr::filter(.data[[date_col]] <= end_date) |>
...
impute_missing_lab_geo <- function(lab_data, afp_data = NULL) {
  if (!"EPID" %in% names(lab_data) && "EpidNumber" %in% names(lab_data)) {
    lab_data <- dplyr::rename_with(lab_data, recode, EpidNumber = "EPID")
  }
...
apply_lab_cleaning_updates <- function(lab_data) {
  ...
  lab_data |>
    dplyr::mutate(seq.lab = dplyr::case_when(
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab == "Cameroon" ~ "NICD-South Africa",
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab == "ETHIOPIA/ KEMRI-Kenya" ~ "UVRI-Uganda",
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab %in% c("Ibadan-Nigeria, Maiduguri-Nigeria", "Nigeria") ~ "Ibadan-Nigeria",
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab == "KEMRI-Kenya" ~ "UVRI-Uganda",
      country == "UGANDA" & DateStoolCollected >= lubridate::as_date("2025-02-01") ~ "UVRI-Uganda",
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab == "Senegal" ~ "NICD-South Africa",
      seq.lab == "CDC-Atlanta" & DateStoolCollected >= lubridate::as_date("2025-02-01") & culture.itd.lab == "Varied (KEMRI-Kenya/ Oman/ Jordan)" ~ "Varied (UVRI/ Oman/ Jordan)",
      .default = seq.lab
    )) |>
...
clean_lab_data_regional <- function(lab_data,
  ...
  out <- dplyr::rename_with(lab_data, recode, Name = "country") |>
    ...
    dplyr::left_join(lab_locs |> dplyr::select(country, seq.capacity, seq.lab, culture.itd.lab), by = "country") |>
